$(document).ready(function(){
	...
	var windowScroll = $(this).scrollTop();
		...
		if ($('.bottom2_holder').length >= 1) {

			var fixbtmbg = $('.bottom2_holder').offset().top;
				if (windowScroll > fixbtmbg && window_width > 1024){

				$('#bottom2').addClass('fixcon');
					} else {
				$('#bottom2').removeClass('fixcon');
				}
		}

		// if ($('#bottom2_innder').length >= 1) {

		// 	var fixbtmbg = $('#bottom2_innner').offset().top;
		// 		if (windowScroll > fixbtmbg && window_width > 1024){

		// 		$('#bottom2_innner').addClass('fixbtmbg');
		// 			} else {
		// 		$('#bottom2_innner').removeClass('fixbtmbg');
		// 		}
		// }
	...
	$('#submit_formmessage .form_btn').on('click', function(){
		...
		if(grecaptcha.getResponse() == "") {
			var $recaptcha = document.querySelector('#g-recaptcha-response');
				...
				$('.g-recaptcha iframe').addClass('FormReq1').attr('id','recaptcha');
		}
	});	
	...
	$(document).mousemove(function(event) {
		...
		mouseYpercentage = Math.round(event.pageY / windowHeight * 3050);
		
		$('.radial-gradient').css('background', 'radial-gradient(at ' + mouseXpercentage + '% ' + mouseYpercentage + '%,  #ff9100, #ff9100 , #ff5100, #ff0000  )');
	  });
});
